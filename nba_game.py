...
def play(current_ball_handler,action,team_1_players,team_2_players):
    ...
    while action.lower() == "pass" and stays == True:
        ...
        ## HOMEWORK:
        #  Instead of having people type in the names of the people to pass
        #  to, try to build a menu prompting people for input.

        ## EXAMPLE:
        #  A) Lebron James
        #  B) Anthony Davis
        #  ...
        #  Who do you want to pass it to:
        #  and then the user of your program can just type in "A" instead of
        #  Lebron James
...
def winner(team_1_points,team_2_points,team_1_name,team_2_name):
    if team_1_points > team_2_points:
        print("{} has won".format(team_1_name))
    elif team_1_points < team_2_points:
        print("{} has won".format(team_2_name))
    else:
        print("{} and {} have tied".format(team_1_name, team_2_name))

def main():
    team_file = open("nba.txt","r")
    ...
    ## HOMEWORK:
    #  Convert this into a function that just returns all_teams
    all_teams = {}
    current_team_name = None
    ## HOMEWORK:
    #  Research Regular Expressions and how to use them
    ## HINTS:
    #  ^       == Start of Line
    #  $       == End of Line
    #  ()      == Capture Group
    #  (?P<x>) == Named Capture Group with Name of "x"
    team_extractor = re.compile(r"^Team: (?P<team_name>.*)$")
    for line in team_file:
        clean_line = line.strip()
        extraction = team_extractor.match(clean_line)
        if extraction:
            team_name = extraction.group('team_name')
            all_teams[team_name] = []
        elif not clean_line == "":
            # that means we are on a player clean_line
            all_teams[team_name].append(clean_line.split(": ")[1])

    ## HOMEWORK:
    #  below are the same exact code only with different desired_team_x
    #  variables, how could we clean this up so that we don't type out the same code
    #  twice?
    if desired_team_1 in all_teams:
        team_1_players = all_teams[desired_team_1]
    else:
        print("{} is not a valid team!".format(desired_team_1))
        exit()

    if desired_team_2 in all_teams:
        team_2_players = all_teams[desired_team_2]
    else:
        print("{} is not a valid team!".format(desired_team_2))
        exit()
            
    print("team_1", desired_team_1)
    print("team_2", desired_team_2)

    ## HOMEWORK:
    #  Please convert this to using "format" on strings instead of multiple
    #  parameters to a print statement
    ...
    ## HELP: Why are we going to a range of 5? Are there only 5 rounds?
    ## HOMEWORK:
    #  This section emulates 5 rounds of play, with 1 round being defined
    #  as the both teams having a turn with the ball.
    #  You genericise the actions by re-assigning current_x variables.

    #  Instead of genericising within the loop, could you convert this to a generic
    #  function that you pass in the the teams, with the first team being the current
    #  and the second being the opposing?

    ## FUTURE HOMEWORK:
    #  Research Classes and develop a Team class, which can have an array
    #  of players as one attribute and a name as another attribute
    for i in range(5):
        current_ball_handler = team_1_players[0]
        current_team = desired_team_1
        ...
        print("{} can either pass or shoot".format(current_ball_handler))
        action = input("What does {} choose to do: ".format(current_ball_handler))
        ...
        print("{} has {} points".format(desired_team_1, team_1_points))
        
        print("Now the other team has the ball")
        current_ball_handler = team_2_players[0]
        current_team = desired_team_2
        ...
        print("{} has {} points".format(desired_team_2, team_2_points))
